    ...
    if(pushData.data.swrve && pushData.data.swrve._p) {
      ...
      event.waitUntil(
        Promise.all([
          ...
          new Promise(() => { sendClientMessage(event, eventTypes.swrvePushReceived, {}) }),
          self.registration.showNotification(builtNotification.data.swrve.title, builtNotification)
        ])
      ); 
    }
  }
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    Promise.all([
      ...
      new Promise(() => { sendClientMessage(event, eventTypes.swrvePushNotificationClicked, bodyPayload) })
    ])
  ); 
});
...
self.addEventListener('notificationclose', (event) => {
  event.waitUntil(
    Promise.all([
      new Promise(() => { sendClientMessage(event, eventTypes.swrvePushNotificationClosed, {}) })
    ])
  ); 
});
